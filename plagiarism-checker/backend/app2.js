...
async function checkPlagiarismPerURL(queryText) {
  // Proses teks untuk output dan keyword
  const processedTokens = tokenizeText(queryText || '');
  const processedText = processedTokens.join(' ');
  const allInputTerms = getTokensAndNGrams(queryText || '');
  const tf = computeTF(allInputTerms);
  const totalTerms = allInputTerms.length;
  ...
  try {
    ...
    if (!tasks || tasks.length === 0) return { results: [], processedText, topKeywords };
    const resultsApi = tasks[0]?.result;
    if (!resultsApi || resultsApi.length === 0) return { results: [], processedText, topKeywords };

    const urls = resultsApi
      .flatMap(result => result.items.map(item => item.url))
      .filter(url => url);
    if (urls.length === 0) return { results: [], processedText, topKeywords };
